...
\begin{itemize}
    ...
    \item \textbf{Dinámica Oceánica (CMEMS GLORYS12V1):} Proporciona los tensores tridimensionales de velocidad de corrientes $(u, v)$ y el afloramiento vertical $w_0$.
...
\begin{equation}
\text{Crecimiento} = \mu_{max} \cdot f_{light}(z) \cdot f_{nutrients}(T) \cdot \hat{C} \quad ; \quad f_{light}(z) = e^{-k_e |z|}
\end{equation}
Donde la tasa máxima de crecimiento ($\mu_{max}$), el coeficiente de atenuación lumínica ($k_e$) y la tasa de mortalidad ($m$) son aprendidos a partir del decaimiento natural de la biomasa observada. La pérdida física minimiza $\mathcal{L}_{PDE} = \frac{1}{N} \sum ||\mathcal{F}_\theta||^2$.
